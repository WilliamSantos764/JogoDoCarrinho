...
def reiniciar_jogo():
    ...
    global frame_index, frame_timer, insetos, facas, estado_jogo
    player = pygame.Rect(100, ALTURA_CHAO, 50, 50)
    vidas = 3
    morrendo = False
    morte_index = muerte_timer = ataque_index = ataque_timer = 0
    atacando = False
...
while running:
    ...
    if estado_jogo == "menu":
        ...
        if not atacando:
            if keys[pygame.K_a]:
                atacando = True; ataque_index = ataque_timer = 0; direcao_ataque = "frente"
            elif keys[pygame.K_w]:
                atacando = True; ataque_index = ataque_timer = 0; direcao_ataque = "cima"
        if keys[pygame.K_s] and len(facas) < 5:
            facas.append(Faca(player.x + 50, player.y, direcao=1))
        ...
        if atacando:
            ...
            if atacando: velocidade //= 2
            movendo = False
            if keys[pygame.K_LEFT]:
                player.x -= velocidade; movendo = True
            if keys[pygame.K_RIGHT]:
                player.x += velocidade; movendo = True
            player.y = ALTURA_CHAO
            frames_atuais = correr_frames if velocidade == vel_correndo else andar_frames
            if movendo:
                frame_timer += 1
